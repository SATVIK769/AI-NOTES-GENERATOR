...
function Sidebar({result}) {
    ...
    if(!result || !result.subTopics || !result.questions || !result.questions.short || !result.questions.long || !result.questions.diagram){
        return null;
    }
    ...
    <div className='bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-6'>
      ...
      <section className='rounded-lg bg-yellow-50 border border-yellow-200 p-3'>
        ...
        <div className='mb-4 rounded-lg bg-blue-50 border border-blue-200 p-3'>
            ...
            <ul className='list-disc ml-4 text-sm text-gray-700 space-y-1'>
                {result.questions.diagram && <li>{result.questions.diagram}</li>}
            </ul>
        </div>
      </section>
    </div>
